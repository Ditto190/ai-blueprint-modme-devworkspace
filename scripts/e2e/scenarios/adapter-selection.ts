...
async function run(t: Runner) {
  ...
  t.git("switch", "-c", "chore/blueprint-setup");
  t.installBlueprint("--all");

  const manifestPath = "blueprint/.state/manifest.json";
  ...
  const result = t.agent(
    "/onboard\n\nUse Efficient implementation style, commit Blueprint workflow files, keep the installer-selected adapters unchanged, do not add tests or CI, and do not commit. Finish the onboarding report.",
    { maxTurns: 50 }
  );
  ...
  t.check("the report does not relabel the selection as all adapters", !/keep all adapters|all four (?:adapters|tools) (?:are )?(?:installed|selected)/i.test(result.resultText));
  t.check("Onboard created no commit", t.git("rev-parse", "HEAD") === rootCommit);
}

export default {
  name: "adapter-selection",
  description: "Onboard preserves a Claude and Codex manifest without relabeling the shared tree as every compatible tool",
  run
};
